# 3주차 활동지 / Week 3 Worksheet

**도메인 문제 발굴 / Domain problem discovery**

- 작성일 / Date:
- 참여자 / Present:

---

> 이번 주는 **문제를 하나로 정하지 않습니다. ** 문제 후보 3개를 고려합니다.
하나를 고르는 것은 다음주에 결정합니다.
> 

---

## 1. 문제 후보 3개 / Three problem candidates

> 네 칸이 다 채워지지 않으면 아직 문제가 아니라 막연한 아이디어입니다.
사용자는 구체적으로 기입하세요.
> 

| 네 요소 Four elements | 후보 A | 후보 B | 후보 C |
| --- | --- | --- | --- |
| 사용자 Who | 재학생/교직원 | 재학생 | 학교 시스템이 낯선 국내 신입생과 외국인 유학생 |
| 상황 When/Where | 교내에 있는 식당이나 카페, 문구점에 방문 하고자 하는 상황 | 축구나 게임을 하고 싶은데 할 사람이 없다. | 입학 전 2월부터 개강 후 3주까지, 주로 모바일로 틈틈이 사용 |
| 페인포인트 What — 무엇이 **얼마나** | 운영 시간이 제각각이라 방문하는데 헛걸음 가능성 리스크가 있고 정보가 흩어져 있어 정보 수집의 시간 소요가 있다 | 혼자 즐길 수 없는 활동을 하고싶을 떄 같이 할 사람이 없어 아예 하지를 못함  |  수강신청 규칙을 몰라 원하는 과목을 놓침 / 교수님께 보낼 이메일 형식을 몰라 한 통에 30분 이상 걸림 / 건물 약칭과 호실 코드를 몰라 첫 주에 지각함 / 술자리에서 거절하는 법을 몰라 원치 않는 음주를 함 |
| 성공 기준 Done | 가고자하는 장소의 운영시간을 빠르게 체크 | 매칭 성공 |   시뮬레이션 완주율 70% 이상 / 수강신청 관련 학사팀 문의 30% 감소 / 개강 첫 주 "강의실 못 찾음" 응답이 미참여자의 절반 수준 |

**한 문장으로 / One sentence each** — 네 칸을 이어 붙여 소리 내어 읽어 보세요.
형식: **[사용자]는 [상황]에서 [페인포인트] 때문에 [손해]를 본다.**
읽었을 때 어색하면 그 칸이 아직 덜 채워진 것입니다.

- A: 나는 교내 식당·카페·문구점 등을 이용하려고 할 때, 마감 시간을 알지 못해 방문했다가 헛걸음한 적이 있었다.
- B:재학생은 축구나 게임을 하고싶은데 같이 할 사람이 없는 상황에서 같이할 사람이 없어 손해를 본다.
- C: 학교 시스템이 낯선 신입생과 유학생은 입학 직후 첫 학기 준비 과정에서 학교생활의 암묵적인 규칙을 몰라서, 원하는 과목을 놓치거나 첫 주에 지각하는 손해를 본다.

> 무엇을 **만들지는 쓰지 않습니다.**  기능은 4~5주차에 정합니다. / Describe the problem, not the solution.
> 

> 성공 기준은 지금은 말로 써도 됩니다. "몇 초 이내" 같은 정확한 문장으로 다듬는 것은
4주차에 EARS로 합니다. / Plain words are fine for now — we formalise these next week.
> 

---

## 2. 사용자 말고 또 누가 / Who else is affected

> 후보 3개 각각에 대해 한 줄씩. 역할이 아니라 사람으로 씁니다.
"교직원"(X) → "학사 공지를 올리는 학과 조교"(O)
"없다"고 적어도 됩니다. 대신 왜 없는지 쓰세요. 억지로 채우지 마세요.
> 
> 
> 이건 **인터뷰 대상을 정하기 위한 것**입니다.
> 주제가 확정되는 4주차에, 살아남은 후보 하나만 `ETHICS.md` 1절에 올립니다.
> 

| 질문 | 후보 A | 후보 B | 후보 C |
| --- | --- | --- | --- |
| 이 안에 **누구의 정보**가 들어가는가<br>그 사람이 우리 사용자와 같은 사람인가 | 늦게 팀플을 하고 학식을 먹으려고 하는 학생 | 협동 활동을 하고 싶은 대학생 |  **없다.** |
| 쓰고 싶어도 **못 쓰는 사람**은 누구인가 | 교내 시설은 누구나 사용가능하기에 없다. | 없다 | 시각장애 학생 |

---

## 3. 확인할 가정과 확인 계획 / Assumptions and how to check them

> 지금 사실이라 믿고 있지만 아직 확인하지 않은 것을 적고, **어떻게 확인할지** 정합니다.
> 

**먼저 고르세요 / Pick the right method**

| 방법 | 이럴 때 씁니다 |
| --- | --- |
| **인터뷰** | 물어야만 알 수 있는 것 — 기억, 판단, 겪은 일, 왜 그렇게 했는지 |
| **관찰** | 가서 보면 알 수 있는 것 — 줄 길이, 걸리는 시간, 실제로 하는 행동 |
| **기존 데이터** | 이미 어딘가 쌓여 있는 것 — 학사 통계, 게시글, 앱 리뷰 |
| **직접 해보기** | 우리가 재현할 수 있는 것 — 그 과정을 실제로 한 번 해본다 |

> *Don't interview for what you can observe.*
> 

| # | 후보 | 우리가 사실이라 믿는 것 | 방법 | 누구에게 / 무엇을 | 담당 | 기한 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | A | 확실하지 않은 것에 에너지 소비를 하기 싫다. | 인터뷰 | 재학생/교직원 교내 시설 중 운영시간을 몰라서 찾아본 경험이 있는가, 교내 시설을 열었다고 생각하고 가봤더니 운영하지 않았던 적이 있는가. | 최지범 | 4주차 |
| 2 | B | 다수의 인원이 모여야할 수 있는 활동을 사람이 없어서 하지 못하는 사람이 있을 것이다 | 인터뷰 | 재학생 / 협동 활동을 하고 싶었는데 같이 할 사람이 없어 하지못하 경험이 있는지 | 김형인 | 4주차 |
| 3 | C | 신입생은 학점 제한과 장바구니 규칙을 몰라서 원하는 과목을 놓친다 | 기존 데이터 | 학교 커뮤니티의 수강신청 실패 게시글 30개를 원인별로 분류 | 강준우 | 4주차 |
| 4 | C | 교수님께 보내는 이메일 한 통에 30분 이상 걸린다 | 관찰 | 신입생 3명에게 결석 사유 메일을 실제로 쓰게 하고 소요 시간 측정 | 강준우 | 4주차 |
| 5 | C | 건물 약칭과 호실 코드만 보고는 강의실을 찾기 어렵다 | 직접 해보기 | 가본 적 없는 건물의 강의실을 시간표 코드만 보고 찾아가며 시간과 헤맨 지점 기록 | 강준우 | 4주차 |

> **인터뷰를 쓴다면** 팀원 외 3명 이상,이 문제를 겪고 있는 당사자를 포함하세요.
3명은 **주제를 거르기 위한 최소치**입니다. 통계가 아닙니다.
3명 중 2명이 인정했다고 해서 "66%가 문제를 겪는다"고 쓰면 안 됩니다.
숫자로 주장하려면 훨씬 많이 필요합니다.(설문지를 통한 결과 도출) / Three people is a filter, not a statistic.
> 

**지금은 어떻게 하고 있는가 / What do they do today?**

> 불편해도 사람들은 이미 나름대로 하고 있습니다. 예) 문제를 공유하고 싶은 문제가 있을때 카톡 전송, 엑셀 파일 메일로 보내기, 그냥 포기하기 — **도구가 아니어도 답입니다.**
> 
- **지금은 어떻게 하고 있는가** (도구가 없으면 "그냥 포기한다"도 답): A:직접찾아가본다 B:아는 사람끼리 최대한 모아서 진행하려고한다. C:직접 발품팔아가며 정보를 얻어간다.
- **그래서 충분한가** — 충분하지 않다면 무엇이 부족한가: 효율이 너무 떨어지고 B의 경우는 원하는 활동을 하지못하게된다.

**인터뷰를 한다면, 물어볼 질문 3개 / If you interview, your three questions**

> 과거형으로. "쓰시겠어요?"(X) → "최근에 그런 적 있으셨어요?"(O)
미래 의향을 물으면 대부분 "네"가 나옵니다.
> 
1. 교내 시설 중 운영시간을 몰라서 찾아본 경험이 있는가.

2. 교내 시설을 열었다고 생각하고 가봤더니 운영하지 않았던 적이 있는가.

3. 축구나 게임 등 활동을 하고 싶었는데 같이 할 사람이 없어 하지못한 경험이 있는가.

**관찰·데이터·직접 해보기를 한다면 / If you observe instead**

**3번 (수강신청 실패 게시글)**

- 가정이 맞다고 볼 조건: 30개 중 '학점 제한·장바구니 규칙 오해'가 원인인 글이 10개 이상(약 33% 이상)이거나, 원인별 순위에서 1~2위 안에 든다.
- 가정이 틀렸다고 볼 조건: 5개 이하(약 17% 이하)이고, 서버 지연이나 정원 마감 같은 다른 원인이 훨씬 많다.
- 기준 근거: 원인이 서너 가지로 나뉜다고 보면, 3분의 1 이상은 "주요 원인"이라고 말할 수 있는 수준이에요.

**4번 (결석 사유 메일 작성)**

- 가정이 맞다고 볼 조건: 3명 중 2명 이상이 30분을 넘긴다.
- 가정이 틀렸다고 볼 조건: 3명 모두 20분 이내에 끝낸다.
- 기준 근거: 가정 자체가 "30분 이상"이라서 그 숫자를 그대로 기준으로 썼어요. 3명이라 표본이 작으니, 과반(2명)이 넘으면 맞는 쪽으로 봅니다.

**5번 (강의실 찾아가기)**

- 가정이 맞다고 볼 조건: 찾아간 강의실 3곳 중 2곳 이상에서 입구부터 도착까지 10분을 넘기거나, 잘못 간 횟수가 2회 이상 나온다.
- 가정이 틀렸다고 볼 조건: 모든 강의실에 5분 이내로 도착하고, 잘못 간 적이 없다.
- 기준 근거: 쉬는 시간이 보통 10~15분이라, 건물 안에서만 10분이 걸리면 실제로 지각할 위험이 있다고 판단했어요.

---

## 4. AI 사용 기록 / AI use log → `PROMPTS.md`

> **전부 남기지 않습니다.** AI가 만든 것이 **산출물에 실제로 들어갔을 때만** 남깁니다.
Log only what actually made it into your work — not every question you asked.
> 

| 상황 | 기록? |
| --- | --- |
| "EARS가 뭐야?" 같은 개념 질문, 번역, 오타 수정 | 안 함 |
| AI가 뽑아준 인터뷰 질문을 3절에 옮겨 적음 | **함** |
| AI에게 후보 아이디어를 받아 1절에 반영함 | **함** |
| 받았지만 안 쓰기로 한 것 중, 판단이 오래 걸린 것 | **함** |

**주당 최대 3건.** 3건을 넘으면 가장 중요한 3건만 고릅니다. 억지로 채우지 마세요.
나머지는 한 줄로 / Summarise the rest in one line:

`PROMPTS.md`에 남길 것 — 건당 네 줄이면 충분합니다

- 무엇을 하려고 썼는가
- 넣은 프롬프트 **원문 그대로** — 요약하지 않습니다
- 나온 것 중 **쓴 것 / 버린 것**
- **버렸다면 왜 버렸는가** — 이 한 줄을 봅니다

> 주의 / Caution
AI가 만들어준 문제 후보에는 **사용자가 없습니다.** 그럴듯한 문장만 있습니다.
그대로 1절에 옮기면 '가짜 사용자형'이 됩니다. 사용자는 여러분이 찾아야 합니다.
> 
- [x]  해당 건을 `PROMPTS.md`에 기록했다 / Logged in `PROMPTS.md`
- [ ]  산출물에 들어간 AI 결과물이 없다 / Nothing from AI made it into our work

---

> 수업 종료 시 커밋하세요 / Commit this at the end of class
`git add docs/week-03.md PROMPTS.md && git commit -m "docs: 3주차 활동지 작성"`
>
